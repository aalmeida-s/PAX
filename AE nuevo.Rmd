...
Una vez encontrado, en nuestro caso q = 16, ajustaremos el modelo para eliminar los factores de confusión y volveremos a generar un *plot* de nuestras muestras.

```{r}
## fit the OUTRIDER model
ods_sizeFactors <- estimateSizeFactors(ods_pass_filter)
ods_bestq <- controlForConfounders(ods_sizeFactors, q= 16, iterations=3)
```
...
Hacemos varios cribados de menos a más exigente 
```{r}
# get results
...
after <- data.table(when = "After", BCV = 1/sqrt( theta(fit(ods_f_n))))
